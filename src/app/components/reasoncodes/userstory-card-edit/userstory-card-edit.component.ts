...
export class UserstoryCardEditComponent implements OnInit {
  ...
  onSaveAll() {
    ...
    if (
      ...
      this.editUSData.ftes != "-----" &&
      !pattern.test(this.editUSData.ftes)
    ) {
    ...
    if (
      this.editUSData.dev_hrs != null &&
      this.editUSData.dev_hrs != "-----" &&
      !pattern.test(this.editUSData.dev_hrs)
    ) {
  ...
  onUpdateProductivity() {
    this.productivity = (
      parseFloat(this.editUSData.ftes) / parseFloat(this.editUSData.dev_hrs)
    ).toFixed(1);
  }
